...
const BP: Record<InstitutionKind, BlueprintGroup[]> = {
  HEALTH: [
    ...
    G('portal', 'Citizen Health Portal', 'Citizen digital health ecosystem', [
  ...
  FINANCE: [
    ...
    // ── Treasury reference-tier expansion (21 new domains) ────────
    G('fiscal-command', 'Fiscal Command', '⌘ I — sovereign fiscal posture', [S('Directives in force', 'command')]),
    G('single-account-overview', 'Treasury Single Account', '⌘ II — TSA position with sub-ledgers', [S('Sub-ledger composition', 'finance')]),
    G('macro-stability', 'Macro Stability Monitor', '⌘ III — indicators feeding fiscal policy', [S('Macro indicators', 'analytics')]),
    G('fiscal-forecasting', 'Fiscal Forecasting', '⌘ IV — 25-year sovereign trajectory', [S('Debt · liquidity · FX trajectories', 'analytics')]),
    G('appropriation-ledger', 'Appropriation Ledger', '⎯ I — budget vs spend by ministry', [S('Ministry execution statement', 'finance')]),
    G('sub-ledger-reconciliation', 'Sub-Ledger Reconciliation', '⎯ II — three-way TSA reconciliation', [S('TSA ↔ rail ↔ ministry', 'audit')]),
    G('daily-statement', 'Daily Fiscal Statement', '⎯ III — end-of-day position', [S('Inflows · outflows · balance', 'finance')]),
    G('expenditure-control', 'Expenditure Control', '⎯ IV — encumbrance & release', [S('Encumbrance pipeline', 'finance')]),
    G('tax-revenue', 'Tax Revenue Collection', '▤ I — live tax collection', [S('Stream statement', 'finance')]),
    G('customs-revenue', 'Customs Revenue', '▤ II — border duty collection', [S('Duty by stream', 'finance')]),
    G('taxpayer-registry', 'Taxpayer Registry', '▤ III — citizen + entity registry', [S('Category composition', 'registry')]),
    G('e-invoicing', 'e-Invoicing', '▤ IV — real-time invoice validation', [S('VAT reconciliation', 'workflow')]),
    G('risk-based-audit', 'Risk-Based Audit', '▤ V — AI-driven audit selection', [S('Selection criteria', 'audit')]),
    G('disbursement-vouchers', 'Disbursement Vouchers', '⊡ I — live voucher register', [S('Voucher register', 'finance')]),
    G('procurement-boards', 'Procurement Boards', '⊟ I — open tenders', [S('Tenders by sector', 'workflow')]),
    G('contract-registry', 'Contract Registry', '⊟ II — awarded contracts', [S('Active contracts', 'registry')]),
    G('vendor-registry', 'Vendor Registry', '⊟ III — supplier registration', [S('Vendor status', 'registry')]),
    G('milestone-escrow', 'Milestone Escrow', '⊟ IV — milestone disbursement escrow', [S('Escrow vouchers', 'finance')]),
    G('payments-rail', 'GovPay Rail', '◴ I — instant transfer & batch disbursement', [S('Channel utilisation', 'logistics')]),
    G('cbdc-operations', 'CBDC Operations', '◴ II — retail & wholesale CBDC', [S('Circulation & wallets', 'finance')]),
    G('settlement-reconciliation', 'Settlement Reconciliation', '◴ III — bank settlement match', [S('Three-way match', 'audit')]),
    G('sovereign-reserves', 'Sovereign Reserves', '◴ V — FX reserves position', [S('Composition', 'finance')]),
    G('public-debt', 'Public Debt Portfolio', '⊟ V — sovereign debt instruments', [S('Instrument composition', 'finance')]),
    G('fiscal-audit', 'Fiscal Audit Trail', '✓ I — hash-chained audit trail', [S('Regional risk', 'audit')]),
    G('anti-fraud', 'Anti-Fraud Intelligence', '✓ II — AI-driven detection', [S('Explainable signals', 'intelligence')]),
    G('taxpayer-portal', 'Taxpayer Portal', '◇ I — citizen tax filing', [S('Filing experience', 'citizen')]),
    G('public-budget-dashboard', 'Public Budget Dashboard', '◇ II — anonymised execution view', [S('Public lookups', 'citizen')]),
  ],
  EDUCATION: [
