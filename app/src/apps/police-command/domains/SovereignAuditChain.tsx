...
// Police domain — Sovereign Audit Chain. Tamper-evident command audit
// ledger: cross-scope integrity, the Police command chain verification,
// and the recent audit trail for this Police instance.

import * as React from 'react';
import { auditStats, auditTrail, verifyChain, subscribe, version } from '@/services/audit-ledger';

export function SovereignAuditChain({ id }: { id: string }) {
  const scope = `${id}:strategic-command`;
  ...
  const tiles: { l: string; val: string; ok: boolean }[] = [
    ...
    { l: 'Police chain', val: chain.intact ? `OK · ${chain.entries}` : `BROKEN @${chain.brokenAt}`, ok: chain.intact },
  ];
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="rounded-[3px] border border-line bg-surface">
        ...
        {trail.length === 0 ? (
          <p className="px-2 py-2 text-[11px] text-ink-muted">No ledger entries recorded for this Police instance yet.</p>
        ) : (
