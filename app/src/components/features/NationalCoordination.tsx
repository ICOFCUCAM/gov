...
import { TONE, ACCENT, seed, Spark, Panel, TerritoryHeat } from '@/components/features/SituationRoom';
...
export function NationalCoordination() {
  ...
  const sp = (k: string, n = 18, lo = 30, hi = 80) =>
    Array.from({ length: n }).map((_, i) => lo + seed(`s:${k}:${i}:${epoch}`) * (hi - lo));
