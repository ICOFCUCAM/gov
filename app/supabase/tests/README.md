...
| `operational_stats_test.sql` | The operational aggregates — `escalation_response_stats` (MTTA/MTTR), `dispatch_response_stats` (ack/on-scene/close medians), `posture_stats` (latest + averages), `work_item_flow_stats` (cycle time + backlog), `escalation_response_trend` (weekly MTTA/MTTR over a fixed same-week dataset) — over fixed datasets. |
