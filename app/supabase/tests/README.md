...
| `accountability_stats_test.sql` | The published aggregate arithmetic — `service_sla_stats` (counts, median turnaround, rated count, avg satisfaction), `appeals_stats` (counts incl. withdrawn, pending excludes withdrawn, median days-to-decision), and `appeals_trend` (weekly decided buckets + median decision days, fixed same-week dataset) — over a fixed fabricated dataset. |
...
| `operational_stats_test.sql` | The operational aggregates — `escalation_response_stats` (MTTA/MTTR), `dispatch_response_stats` (ack/on-scene/close medians), `posture_stats` (latest + averages), `work_item_flow_stats` (cycle time + backlog), `escalation_response_trend` (weekly MTTA/MTTR) and `dispatch_response_trend` (weekly ack/on-scene/close medians), both over fixed same-week datasets — over fixed datasets. |
