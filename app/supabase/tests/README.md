...
| `accountability_stats_test.sql` | The published aggregate arithmetic — `service_sla_stats` (counts, median turnaround, rated count, avg satisfaction), `appeals_stats` (counts, median days-to-decision), and `appeals_trend` (weekly decided buckets + median decision days, fixed same-week dataset) — over a fixed fabricated dataset. |
