...
- **`anon_security_definer_function_executable`** — 5 (was 34); the remaining five are deliberate Public Observatory reads/attestations, each safe for an anonymous caller: `civicos_current_actor` (returns null for anon sessions), `civicos_verify_audit_chain` (public chain-integrity read), `civicos_record_witness_attestation` (external "I saw the chain at seq N" attestation), `civicos_service_sla_stats` and `civicos_appeals_stats` (charter-level aggregate service / contestation stats — no row-level data).
