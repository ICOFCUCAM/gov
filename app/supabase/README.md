...
| `20260521430000_civicos_telemetry_stream_stats.sql` | `telemetry_stream_stats(p_stream_id, p_hours)` — per-stream distribution (count, min/max/avg/median/p95/stddev), latest value, and warn/alert breach counts over a window, using the same `value >= threshold` rule as the auto-escalate trigger. Authenticated-tier (samples are not public); powers the statistics panel on the telemetry stream detail. |
| `20260521440000_civicos_telemetry_fleet_status.sql` | `telemetry_fleet_status(p_charter_id, p_stale_minutes)` — for every active stream, its latest value + computed status (alert / warn / stale / ok, worst-first), where `stale` means no sample within the window (a silent sensor). Authenticated-tier; powers the at-a-glance Fleet status panel on the Telemetry Wall. |
| `20260521450000_civicos_telemetry_stale_escalation.sql` | `escalate_stale_telemetry_streams(p_stale_minutes)` — records a 'minor' escalation per active stream with no sample inside the window (a silent sensor emits nothing to trip the threshold trigger, so this is the only path that surfaces a blind spot). Per-stream 24h idempotency, mirroring the stale-service-request worker. Service-role / platform-tier; paired with `/api/cron/telemetry-stale`. |
| `20260521460000_civicos_telemetry_stream_series.sql` | `telemetry_stream_series(p_stream_id, p_hours, p_buckets)` — downsamples a stream into ≤ N evenly-spaced time buckets (avg/min/max + count per bucket) so the detail can chart 7-/30-day windows at a bounded payload. Authenticated-tier; powers the window selector on the telemetry stream detail. |
| `20260521470000_civicos_escalation_response_stats.sql` | `escalation_response_stats(p_charter_id, p_days)` — per-source-charter incident-response stats: total / acknowledged / resolved / open counts, MTTA (median minutes to acknowledge), MTTR (median hours to resolve), p90s, and oldest-open age. The incident-response analogue of service_sla_stats. Authenticated-tier; powers the Response times panel on the Escalation Floor. |
| `20260521480000_civicos_posture_stats.sql` | `posture_stats(p_charter_id, p_days)` — per-charter posture summary over a window: latest posture + readiness/stress, plus average and extreme readiness/stress, most-stressed first. Authenticated-tier; the Posture Board's comparative-stress panel now shows the 30-day average alongside the instantaneous reading (sustained vs spot stress). |
| `20260521490000_civicos_work_item_flow_stats.sql` | `work_item_flow_stats(p_workflow_id, p_charter_id, p_days)` — per-workflow internal throughput: items opened / closed / still-open, median + p90 cycle time (created → closed) in hours, and oldest-open age, backlog-heaviest first. The internal-clock analogue of service_sla_stats. Authenticated-tier; surfaced as per-workflow open/cycle badges on the Workflow Catalogue. |
| `20260521500000_civicos_work_item_stage_distribution.sql` | `work_item_stage_distribution(p_workflow_id)` — open work items for one workflow grouped by `current_stage` (WIP per stage + oldest and median age in stage), busiest stage first, so the bottleneck is obvious. Age uses `updated_at` (time since the item last transitioned). Authenticated-tier; powers the "open work by stage" bar in the Workflow Catalogue. |
| `20260521510000_civicos_officer_workload.sql` | `officer_workload(p_charter_id)` — open work-item load per assignee (with an explicit `(unassigned)` bucket for unowned backlog): open count, urgent/critical count, oldest + median open age, most-high-priority first. Authenticated-tier; surfaces a per-officer "N open · M!" badge on the Officer Directory so a supervisor can spot overload. |
| `20260521520000_civicos_federation_channel_catalog.sql` | `federation_channel_catalog(p_days)` — per (channel, type) federation-event volume + last occurrence over a window, so an operator can see what actually flows on each channel before registering an outbound webhook. Authenticated-tier; powers the Channel activity panel on the Federation Webhooks surface. |
| `20260521530000_civicos_dispatch_response_stats.sql` | `dispatch_response_stats(p_charter_id, p_days)` — per-issuing-charter dispatch response stats over the four-point lifecycle: volume, acknowledged / on-scene / closed counts, open backlog, median time-to-acknowledge (min), median time-to-on-scene (min), median time-to-close (hr), oldest-open age. The emergency-response analogue of the escalation/SLA stats. Authenticated-tier; powers the Response times panel on the Dispatch Board. |
| `20260521540000_civicos_claim_work_item.sql` | `claim_work_item(p_ref)` — the calling officer takes ownership of an open work item (assignee := self), recorded on the item's audit scope. Assignment-only — does NOT advance the workflow stage or touch the transition state machine. Requires a linked officer (auth.uid()); authenticated-tier. Closes the loop from `officer_workload`'s unassigned backlog: the Work Item detail gains a "claim / assign to me" action. |
| `20260521550000_civicos_release_work_item.sql` | `release_work_item(p_ref)` — companion to claim: hand an open item back to the unassigned pool (assignee := null), audit-logged. Only the current assignee or a platform-tier officer may release (you cannot yank another officer's item without a platform role). Assignment-only; authenticated-tier. The Work Item detail gains a "release" action, completing the self-service rebalancing loop. |
| `20260521560000_civicos_set_work_item_priority.sql` | `set_work_item_priority(p_ref, p_priority)` — re-triage an open item's priority (validated against the `civicos.priority` enum), audit-logged. Only the current assignee or a platform-tier officer may; does NOT advance the workflow stage. Authenticated-tier; the Work Item detail gains an inline priority selector. |
| `20260521570000_civicos_reassign_work_item.sql` | `reassign_work_item(p_ref, p_assignee_id)` — supervisor counterpart to claim/release: a platform-tier officer assigns an open item to any active officer (audit-logged, no state-machine change). Gated to platform-tier; target must be an active officer. Authenticated-tier; the Work Item detail gains a "reassign to…" officer picker for platform-tier sessions. |
| `20260521580000_civicos_dispatch_on_scene.sql` | `mark_dispatch_on_scene(p_ref)` — fills the missing middle of the dispatch lifecycle (dispatched → acknowledged → **on_scene** → closed): only ack and close RPCs existed, so `on_scene_at` (and its response-stat) was never reachable. Sets `on_scene_at`, backfills `acknowledged_at`, advances status to `on-scene`. Authenticated-tier (tighter than the older anon-callable ack/close); the Dispatch detail gains an "on scene" action. |
| `20260521590000_civicos_promote_due_directives.sql` | `promote_due_directives()` — flips signed directives whose future `effective_at` has now passed to 'effective' (sign_directive only set 'effective' immediately for already-past dates, so a future-dated directive stayed 'signed' forever). One audit entry per directive on its `directive:<ref>` scope. Service-role only; paired with `/api/cron/promote-directives` and registered in CronStatus. |
| `20260521600000_civicos_advance_appeal_stage.sql` | `advance_appeal_stage(p_ref, p_stage)` — the missing middle of the appeal lifecycle (filed → **admitted** → **heard** → decided): only file_appeal and decide_appeal existed, so `admitted_at`/`heard_at` were dead columns and `appeals_stats`' admitted count was always 0. Advances an open appeal to admitted/heard (heard backfills admitted_at), linked-officer only, audit-logged. The Appeal detail gains "admit" / "hear" actions. |
| `20260521610000_civicos_rate_service_request.sql` | `rate_my_service_request(p_ref, p_satisfaction)` — citizen-scoped rating (1–5) of one's own RESOLVED service request, resolved from `auth.uid()` so a citizen can only rate requests they own. Previously satisfaction was only settable via the unscoped officer-side `update_service_request`. Audit-logged on the citizen's scope; the wallet substrate gains an inline star rating on resolved requests. |
| `20260521620000_civicos_sla_stats_satisfaction.sql` | Extends `service_sla_stats` with `rated` (count of scored resolved requests) and `avg_satisfaction` (mean 1–5) per charter — citizen satisfaction is a service-delivery KPI, so it lives with the SLA aggregate rather than a separate RPC. Drop+recreate (OUT-column change); still anon-callable, aggregate-only. Surfaced as a "rating" column on the Observatory SLA board (+ CSV + open-data API) and a satisfaction stat on the per-charter profile. |
| `20260521630000_civicos_audit_scope_summary.sql` | `audit_scope_summary(p_limit)` — platform-tier/auditor index of every audit scope (entry count, first/last activity, max seq), newest activity first. The chain is per-scope and scopes have proliferated (citizen:/appeal:/directive:/work-item); this enumerates them so the Audit Explorer can offer a directory. Gated to platform-tier (scopes include `citizen:<uuid>`). |
| `20260521640000_civicos_revoke_all_my_consents.sql` | `revoke_all_my_consents()` — citizen "cut off all access" kill switch: revokes every still-granted consent the caller owns in one call, one summary audit entry, returns the count. Scoped to `auth.uid()`; authenticated only. The wallet consents panel gains a confirm-guarded "revoke all access (N)" button. |
| `20260521650000_civicos_my_notifications.sql` | `my_notifications(p_limit)` — citizen's short "needs your attention" feed derived from substrate state: resolved-but-unrated requests (→ rate), appeals decided in the last 30 days (→ view), consents expiring within 14 days (→ extend). Scoped to `auth.uid()`; authenticated. Powers a "Needs your attention" panel on the citizen home. |
| `20260521660000_civicos_data_export_audit_trail.sql` | Adds the citizen's tamper-evident `audit_trail` (hash-chained entries on their `citizen:<id>` scope) to `my_data_export` (version → 2) — the audit log is the citizen's data too and belongs in a complete portability document. Public-wrapper empty-caller fallback kept in sync. |
| `20260521670000_civicos_telemetry_stream_active.sql` | `set_telemetry_stream_active(p_stream_id, p_active)` — decommission/reactivate a telemetry stream. `active` had no setter (define only upserts metadata), so a retired sensor stayed "active" forever and kept tripping stale-sensor escalation. Deactivating drops it from fleet status, the public catalog, and stale escalation. Platform-tier; the Telemetry Wall gains an active/off toggle per stream. |
| `20260521680000_civicos_link_escalation_response.sql` | `link_escalation_response(p_escalation_id, p_dispatch_ref, p_work_item_ref)` — attach the dispatch and/or work item raised in response to an escalation. `escalations.linked_work_item_id`/`linked_dispatch_id` were declared but had no setter, so the incident↔response linkage was unreachable. Officer-gated, audit-logged; the Escalation detail gains a "link dispatch / work item" control. |
| `20260521690000_civicos_dispatch_close_resolves_escalation.sql` | Trigger `dispatch_close_resolves_escalation` — when a dispatch linked to an escalation closes, the escalation it answered auto-resolves (and acknowledges if it hadn't). Completes the incident loop opened by `link_escalation_response`. AFTER UPDATE on the closed_at transition; idempotent. |
| `20260521700000_civicos_work_item_close_resolves_escalation.sql` | Trigger `work_item_close_resolves_escalation` — the symmetric automation: when a work item linked to an escalation closes, the escalation auto-resolves. Whichever response form (dispatch or work item) closes the incident, the escalation stops lingering open. AFTER UPDATE on the closed false→true transition; idempotent. |
| `20260521710000_civicos_work_item_flow_trend.sql` | `work_item_flow_trend(p_workflow_id, p_charter_id, p_weeks)` — weekly throughput trajectory (by closed-week): closed count + median cycle hours. The internal-throughput analogue of `service_sla_trend`; shows whether a workflow is clearing work faster or slower. Authenticated-tier; powers a throughput-trend bar on the Workflow Catalogue. |
| `20260521720000_civicos_appeals_trend.sql` | `appeals_trend(p_charter_id, p_weeks)` — weekly buckets (by decided-week) of decided count + median/p90 days-to-decision over the last N weeks. The contestation analogue of `service_sla_trend`; publishes whether appeal decision times are improving or sliding, not just a snapshot. Anon-callable, aggregate-only. Powers the appeals decision-time trend chart on the Observatory and is added to `/api/public/accountability`. |
| `20260521730000_civicos_escalation_response_trend.sql` | `escalation_response_trend(p_charter_id, p_weeks)` — weekly buckets (by resolved-week) of resolved count + median MTTA minutes + median MTTR hours over the last N weeks. The incident-response trajectory behind `escalation_response_stats`' snapshot; shows whether response is getting faster or slower. Authenticated-tier, aggregate-only; powers a response-time trend bar on the Escalation Floor. |
| `20260521740000_civicos_dispatch_response_trend.sql` | `dispatch_response_trend(p_charter_id, p_weeks)` — weekly buckets (by closed-week) of closed count + median time-to-ack/on-scene (minutes) + time-to-close (hours) over the last N weeks. The command-and-control trajectory behind `dispatch_response_stats`' snapshot. Authenticated-tier, aggregate-only; powers a response-time trend bar on the Dispatch Board. |
| `20260521750000_civicos_withdraw_appeal.sql` | Adds `appeals.withdrawn_at` and `withdraw_my_appeal(p_ref, p_reason)` — the first citizen-initiated appeal exit: scoped to `auth.uid()`'s citizen, only on an appeal they own that is neither decided nor already withdrawn, audit-logged on the citizen scope. Also rebuilds `appeals_stats` so `pending` / `oldest_pending_days` exclude withdrawn rows and a new `withdrawn` count is published, keeping the backlog honest. Citizen wallet gains a "withdraw" action on open appeals. |
| `20260521760000_civicos_cancel_service_request.sql` | Adds `service_requests.cancelled_at` and `cancel_my_service_request(p_ref, p_reason)` — the citizen-initiated service-request exit symmetric to `withdraw_my_appeal`: scoped to `auth.uid()`'s citizen, only on a request they own that is neither resolved nor already cancelled, audit-logged (reason stored on the payload). Also rebuilds `service_sla_stats` so `open` / `oldest_open_hours` exclude cancelled requests and a new `cancelled` count is published. Citizen wallet gains a "cancel" action on open requests. |
| `20260521770000_civicos_expose_exit_columns.sql` | Recreates the `civicos_service_requests` / `civicos_appeals` read views to append the new `cancelled_at` / `withdrawn_at` columns — the explicit-column views had a frozen list so PostgREST never surfaced them, leaving the wallet reads undefined and the officer "open" queue still showing citizen-exited items. With the columns exposed, `listServiceRequestsRows`/`listAppealsRows` `openOnly` now also excludes cancelled/withdrawn. `security_invoker` preserved. |
| `20260521780000_civicos_posture_trend.sql` | `posture_trend(p_charter_id, p_weeks)` — weekly buckets (by snapshot-week) of snapshot count + avg readiness + avg/peak stress over the last N weeks. The posture trajectory behind `posture_stats`' snapshot, completing the stat-trend family (SLA, work-item flow, appeals, escalation, dispatch, posture). Authenticated-tier, aggregate-only; powers a charter-scoped stress-trend bar on the Posture Board. |
| `20260521790000_civicos_consent_footprint_stats.sql` | `consent_footprint_stats(p_charter_id)` — per (charter, scope) data-access footprint: active consents (granted and not past expiry), those expiring within 30 days, and revoked. A SECURITY DEFINER aggregate over the RLS-protected consents table; aggregate-only (no citizen id or row) and anon-callable, so the public can see how much data-access power each institution holds. Only (charter, scope) pairs with an active grant appear. Powers the data-access footprint panel on the Observatory. |
| `20260521800000_civicos_directive_stats.sql` | `directive_stats(p_charter_id, p_days)` — per-charter governance output over a window (by signed date): signed / effective / in-force / rescinded counts + median signed→effective lag in days. Counts a directive only once signed so drafts never leak. Anon-callable, aggregate-only. Powers the governance-output panel on the charter profile. |
| `20260521810000_civicos_directive_trend.sql` | `directive_trend(p_charter_id, p_weeks)` — weekly buckets (by signed-week) of signed count + how many became effective + median signed→effective lag. The legislative-cadence trajectory behind `directive_stats`. Anon-callable, aggregate-only; powers the directive cadence trend on the charter profile and `/api/public/accountability`. |
| `20260521820000_civicos_exit_events.sql` | Recreates `withdraw_my_appeal` / `cancel_my_service_request` to also `publish_event` on the `wallet` channel (`appeal.withdrawn` / `service.cancelled`), targeted at the owning charter, so registered federation webhooks fire on a citizen exit and the channel catalog reflects it. Guards and audit entries unchanged. |
| `20260521830000_civicos_charter_attention.sql` | `charter_attention(p_limit)` — the officer counterpart to `my_notifications`: a charter-scoped "needs attention" feed (service requests open >48h unacknowledged, escalations unacknowledged, appeals pending >7 days, dispatches open + unacknowledged), scoped via `current_officer_charter()`, oldest-first, with each item's age in hours. Excludes citizen-exited (cancelled/withdrawn) work. Authenticated-only; powers the "Needs attention" panel on Officer Home. |
| `20260521840000_civicos_charter_attention_workitems.sql` | Extends `charter_attention` with a fifth arm — open work items originating from the charter with no update in 72h (`workitem_stalled` → action: progress) — so silent stalled workflow items surface alongside the other backlogs. Other arms unchanged. |
| `20260521850000_civicos_federation_edges.sql` | **Phase C.** Persists the cross-ministry dependency graph (archetype-level, matching `institutions.archetype_or_branch`) that previously lived only in TS config, so SQL can resolve a charter's real dependents. Seeded to mirror the declared graph; `list_federation_edges(archetype)` exposes the topology (anon-readable; table RLS-locked). |
| `20260521860000_civicos_escalation_cascade.sql` | **Phase C — the organism reacts.** AFTER-INSERT trigger on `escalations`: a `major`/`national` escalation against a registered institution emits an `escalation.cascade` federation event (on the `escalation` channel, targeted at each dependent charter) along every `provides`/`mutual` edge — the institutions that lose something when the source is impaired. Additive and bounded: publishes events only, never opens escalations (no recursion, no operational blast radius). Surfaced as "Cross-ministry cascades" on the Escalation Floor. |
...
- **`anon_security_definer_function_executable`** — 10 (was 34); the remaining ten are deliberate Public Observatory reads/attestations, each safe for an anonymous caller: `civicos_current_actor` (returns null for anon sessions), `civicos_verify_audit_chain` (public chain-integrity read), `civicos_record_witness_attestation` (external "I saw the chain at seq N" attestation), and the charter-level aggregate stats `civicos_service_sla_stats`, `civicos_appeals_stats`, `civicos_service_sla_trend`, `civicos_appeals_trend`, `civicos_consent_footprint_stats`, `civicos_directive_stats`, `civicos_directive_trend` (no row-level data).
