...
- **`anon_security_definer_function_executable`** — 7 (was 34); the remaining seven are deliberate Public Observatory reads/attestations, each safe for an anonymous caller: `civicos_current_actor` (returns null for anon sessions), `civicos_verify_audit_chain` (public chain-integrity read), `civicos_record_witness_attestation` (external "I saw the chain at seq N" attestation), and the charter-level aggregate stats `civicos_service_sla_stats`, `civicos_appeals_stats`, `civicos_service_sla_trend`, `civicos_appeals_trend` (no row-level data).
